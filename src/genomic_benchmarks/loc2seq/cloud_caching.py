...
from google_drive_downloader import GoogleDriveDownloader as gdd   # TODO: use gdown instead of google_drive_downloader
import shutil


CLOUD_CACHE = {("human_nontata_promoters", 0):          "1VdUg0Zu8yfLS6QesBXwGz1PIQrTW3Ze4",
...
def download_from_cloud_cache(file_key, dest_path, cloud_cache=CLOUD_CACHE, force_download=True):
    ...
    if file_key not in cloud_cache:
        raise ValueError(f"File ID {file_id} not in the cloud cache.")
    
    if force_download and Path(dest_path).exists():
        shutil.rmtree(str(dest_path))

    gdd.download_file_from_google_drive(file_id=cloud_cache[file_key],
