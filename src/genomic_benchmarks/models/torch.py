    ...
    def test_multiclass(self, dataloader, class_count, positive_label = 1):
        ...
        with torch.no_grad():
            for X, y in dataloader:
                ...
                # TODO class_count for cycle approach
                # for class_label in range(class_count):
                #     p[class_label] += (arg_max_pred == class_label).sum().item()
                #     tp[class_label] += ((arg_max_pred == class_label) * (y == class_label)).sum(dim=0).item()
                #     fp[class_label] += ((arg_max_pred == class_label) * (y != class_label)).sum(dim=0).item()
                #     if (p[class_label] != (tp[class_label] + fp[class_label])):
                #         raise Exception("p has to equal tp + fp")
        ...
        accuracy = correct / size
        # print(f"Test metrics: \n Accuracy: {accuracy:>6f}, F1 score: {f1_score:>6f}, Avg loss: {test_loss:>6f} \n")
        print(f"Test metrics: \n Accuracy: {accuracy:>6f}, Avg loss: {test_loss:>6f} \n")
